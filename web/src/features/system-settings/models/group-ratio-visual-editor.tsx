...
function parseSensitiveDetectionGroups(value: string): string[] {
...
function buildGroupPricingRows(
  ...
  const detectionGroups = parseSensitiveDetectionGroups(sensitiveDetectionGroups)
...
function groupPricingSignature(rows: GroupPricingRow[]): string {
  ...
  return JSON.stringify({
    ...
    sensitiveDetectionGroups: parseSensitiveDetectionGroups(
...
export const GroupRatioVisualEditor = memo(function GroupRatioVisualEditor({
  ...
  const registry = useMemo<RegistryEntry[]>(() => {
    ...
    const usableMap = parseUsableMap(userUsableGroups)
    const topupMap = parseRatioMap(topupGroupRatio)
    const detectionGroups = parseSensitiveDetectionGroups(
      sensitiveDetectionGroups
    )
    const names = new Set([
      ...Object.keys(ratioMap),
  ...
  }, [groupRatio, userUsableGroups, topupGroupRatio, sensitiveDetectionGroups])
...
function GroupDetailSheet(props: GroupDetailSheetProps) {
  ...
  const detail = useMemo(() => {
    ...
    const detectionSet = new Set(
      parseSensitiveDetectionGroups(props.sensitiveDetectionGroups)
    )
    ...
    props.sensitiveDetectionGroups,
          ...
          <div className={sideDrawerFormClassName('gap-5')}>
            <section className='space-y-2'>
              ...
              <dl className='space-y-1.5 text-sm'>
                ...
                <div className='flex justify-between'>
                  <dt className='text-muted-foreground'>
                    {t('Violation detection')}
                  </dt>
                  <dd className='font-medium'>
                    {detail.sensitiveDetectionEnabled ? t('Yes') : t('No')}
                  </dd>
                </div>
